...
class ChronelecScroll implements Provider {
...
//    private $generalUrl = 'http://www.matsport.fr.php53-23.ord1-1.websitetestlink.com/masse/index.php?an=2015&code_course=24R&menu=2&type=1&Num_Menu=0&DebutDossard=1&DebutAlpha=A';
    ...
    private $generalUrl = 'http://chronelec.free.fr/classements/_Live_Html/scratch.html';
...
//        $pattern = '|dossard=(\d+)&code_course=24R&an=2015\',\'Matsport Live\');">NR BN</a></div></td><td align=\'left\'><div class=Style9>(.*)</div></td><td align=\'left\'><div class=Style9>(.*)</div></td>|U';
	...
	$pattern = '|<TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER>  (.*)\s+</CENTER></TD><TD><CENTER></CENTER></TD>|Um';
...
//            $matches[7][$k] = trim(str_replace('(C)', '', $matches[7][$k]));
            ...
            $team
                ...
                ->setNumero($matches[3][$k])
                //'numero'   => $matches[3][$k],
                ->setAnnee('2016')
                ->setNom($matches[4][$k])
                ->setTemps('00:00:00') //$matches[8][$k])
                ->setTour($matches[5][$k])
                ->setEcart($matches[6][$k] == '-' ? '00:00:00' : $matches[7][$k])
                ->setDistance($matches[8][$k])
                ->setVitesse('0')
                ->setBestlap($matches[7][$k] == '-' ? '00:00' : $matches[7][$k])
                ->setPoscat($matches[2][$k])
                ->setCategorie('')
                ;

            $equipes[$team->getNumero()] = $team;
        }
